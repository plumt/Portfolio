        ...
        serch_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (input_email.getText().toString().trim().length() < 10) {
                    Toast.makeText(getApplicationContext(), "10자리 이상 입력해 주세요", Toast.LENGTH_SHORT).show();
                    return;
                } else if (!Patterns.EMAIL_ADDRESS.matcher(input_email.getText().toString()).matches()) {
                    Toast.makeText(getApplicationContext(), "이메일 형식으로 입력해 주세요", Toast.LENGTH_SHORT).show();
                    return;
                }
                        ...
                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                            ...
                            public void onComplete(@NonNull Task<Void> task) {
                                if (task.isSuccessful()) {
                                    Toast.makeText(getApplicationContext(), "인증 메일을 전송하였습니다", Toast.LENGTH_SHORT).show();
                                    finish();
                                    overridePendingTransition(R.anim.fab_open,R.anim.fab_close);
                                } else {
